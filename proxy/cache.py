...
# per-cluster threshold — that swap is the whole product.
...
def lookup(
    ...
    """Return a cached response or None. Not raises"""
    params = params or {}
    try: 
        cache = _tenant(tenant_id)

        key=_exact_key(model, prompt, params)
        if (response:= cache.exact.get(key)) is not None: 
            return Hit(response, "L0", 1.0)

        if cache.matrix is None: 
            return None
        query=embed.embed(prompt)
        sims=cache.matrix @ query
        best=int(np.argmax(sims))
        score=float(sims[best])

        if score >= SIMILARITY_THRESHOLD: 
            return Hit(cache.responses[best], "L1", score)
        return None
    except Exception: 
...
def store(
    ...
    try: 
        cache=_tenant(tenant_id)
        cache.exact[_exact_key(model, prompt, params)]=response
        if len(cache.prompts) >= MAX_ENTRIES_PER_TENANT: 
            return
        vec = embed.embed(prompt),reshape(1,-1)
        with _lock: 
            cache.matrix=(vec if cache.matrix is None else np.vstack([cache.matrix, vec]))
...
def stats(tenant_id: str)-> dict[str, int]: 
    cache=_tenant(tenant_id)
    return {"exact_entries":len(cache.exact), "semantic_entries": len(cache.prompts)}
